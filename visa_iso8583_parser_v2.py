...
try:
    from visa_field_details import get_value_detail, format_field_report
except ImportError:
    # Detail module not on path yet - keep the parser usable without it.
    def get_value_detail(label, value):
        return "visa_field_details.py not found - per-value detail lookup unavailable."
...
FIELD_DEFS = {
    ...
    56: {"type": "tlv_dataset", "len_bytes": 1},                # Customer Related Data - spec shows this as
                                                                 # dataset-TLV (1-byte len + dataset ID + 2-byte
                                                                 # dataset len + TLV body), same shape as F34/F104/
                                                                 # F111. UNCONFIRMED against a live sample: spec
                                                                 # also allows a 2-byte-length variant of this same
                                                                 # structure - if live traffic shows garbage here,
                                                                 # switch len_bytes to 2.
    57: {"type": "ans_v"},                                      # Reserved national
    58: {"type": "ans_v"},                                      # Reserved national
    59: {"type": "ans_v"},                                      # National POS geographic data
    60: {"type": "field60"},                                    # Additional POS Information - positional subfields 60.1-60.10 (see FIELD60_SUBFIELDS)
...
FIELD62_SUBFIELDS = {
    ...
    20: 5, 21: 4, 22: 6, 23: 2, 24: 6, 25: 1, 26: 1,
}
FIELD62_TYPE = {
    2: "bcd", 5: "bcd", 8: "bcd", 10: "bcd", 11: "bcd", 12: "bcd", 16: "bcd",
    # F62.20 (Merchant Verification Value) is spec'd as "10 hexadecimal
    # digits" - NOT packed BCD digits 0-9. The old code rendered it with
    # decode_bcd_full, which turns any A-F nibble into '*', silently
    # corrupting real MVV values. Render as plain hex instead.
    20: "hex",
...
# Field 60 (Additional POS Information) - fixed POSITIONAL subfields per
# spec pp. 307-309 (this field is NOT bitmap-driven like F44/F62/F63/F126;
# it's 12 fixed BCD digit positions, some 1 digit wide, some 2). Unused
# trailing subfields are simply absent per spec ("field 60 is truncated to
# the last field"), which decode_field_60 honors by stopping once nibbles
# run out rather than emitting zero-filled placeholders for subfields that
# were never sent.
FIELD60_SUBFIELDS = [
    (1,  "Terminal Type", 1),
    (2,  "Terminal Entry Capability", 1),
    (3,  "Chip Condition Code", 1),
    (4,  "Special Condition Indicator", 1),
    (5,  "Merchant Group Indicator (Not Applicable)", 2),
    (6,  "Chip Transaction Indicator", 1),
    (7,  "Chip Card Authentication Reliability Indicator", 1),
    (8,  "Mail/Phone/Electronic Commerce and Payment Indicator", 2),
    (9,  "Cardholder ID Method Indicator", 1),
    (10, "Additional Authorization Indicators", 1),
]
FIELD60_DESCRIPTIONS = {sub: name for sub, name, _ in FIELD60_SUBFIELDS}
...
def decode_field(fnum: int, data: bytes, pos: int):
    ...
    if t == "field60":
        return decode_field_60(data, pos)
...
# Field 126 subfield byte-lengths, pulled from the spec's "Field 126.N -
# Attributes" sections (pp. 608-640). All are FIXED length except 126.16,
# which carries its own 1-byte inner length prefix (see
# FIELD126_VARIABLE_SUBFIELDS below) - it is handled separately in
# decode_field_126 rather than through this fixed-length table.
FIELD126_SUBFIELDS = {
    5: 8,     # Visa Merchant Identifier - 8 ANS, EBCDIC
    6: 17,    # Cardholder Certificate Serial Number - 1 byte (sig. digit count) + 16 bytes binary
    7: 17,    # Merchant Certificate Serial Number - 1 byte (sig. digit count) + 16 bytes binary
    8: 20,    # Transaction ID (XID) - binary, 20 bytes (confirmed against live samples)
    9: 20,    # CAVV Data - binary, 20 bytes (confirmed against live samples)
    10: 6,    # CVV2 Authorization Request Data - 6 ANS, EBCDIC (confirmed against live samples)
    12: 3,    # Service Indicators - 24-bit bit string, 3 bytes
    13: 1,    # POS Environment - 1 AN, EBCDIC
    15: 1,    # MasterCard UCAF Collection Indicator - 1 ANS, EBCDIC
    18: 12,   # Agent Unique Account Result - 1 (marker) + 5 + 5 + 1 bytes
    19: 1,    # Dynamic Currency Conversion Indicator - 1 ANS, EBCDIC
    20: 1,    # 3-D Secure Indicator - 1 AN, EBCDIC
}

# F126.16 (MasterCard UCAF Field) is variable length: its own 1-byte
# binary length prefix followed by up to 32 bytes of EBCDIC data (max 33
# bytes total per spec p. 636). It cannot go in FIELD126_SUBFIELDS above
# since that table assumes a fixed byte count consumed directly from the
# bitmap-ordered byte stream.
FIELD126_VARIABLE_SUBFIELDS = {16}

# How to render each subfield's raw bytes into the compact/detail value.
# 'ebcdic'         - EBCDIC text
# 'ebcdic_mask_f'  - EBCDIC text with literal 'F'/'f' rendered as '*' (confirmed masking convention for 126.10... wait 126.10 is EBCDIC text below? kept for 126.10 to match prior confirmed behavior)
# 'hex'            - raw hex passthrough (used for encrypted/binary/unconfirmed-bit-meaning subfields)
FIELD126_RENDER = {
    5: "ebcdic",
    6: "hex",             # 1 sig-digit-count byte + 16 binary bytes; rendered as hex of the whole subfield
    7: "hex",
    8: "hex",
    9: "hex",              # encrypted CAVV data - hex passthrough is the safe/confirmed rendering
    10: "ebcdic_mask_f",   # confirmed: literal 'F'/'f' is the switch's masking placeholder
    12: "hex",             # bit-string; individual bit meanings not extracted from spec - hex passthrough
    13: "ebcdic",
    15: "ebcdic",
    16: "ebcdic",          # MasterCard UCAF Field text (variable length, handled separately)
    18: "hex",             # structured (marker + 2x5-byte AN + bitstring) - hex passthrough pending full breakout
    19: "ebcdic",
    20: "ebcdic",
}
...
def decode_field_126(data: bytes, pos: int):
    ...
    for sub in range(1, 65):
        if sub <= len(bits) and bits[sub - 1]:
            if sub in FIELD126_VARIABLE_SUBFIELDS:
                if rpos >= len(rest):
                    parts.append(f"F126.{sub})<truncated>")
                    continue
                sublen = rest[rpos]
                rpos += 1
            ...
            subdata = rest[rpos:rpos + sublen]
            rpos += sublen
            render = FIELD126_RENDER.get(sub, "hex")
            if render == "ebcdic":
                subval = decode_ebcdic(subdata)
            elif render == "ebcdic_mask_f":
                # Confirmed: literal 'F'/'f' is the switch's masking
                # placeholder (mirrors the 0xF-nibble = masked-digit
                # convention used elsewhere).
                subval = decode_ebcdic(subdata).replace("F", "*").replace("f", "*")
            else:
                subval = to_hex(subdata)
            parts.append(f"F126.{sub})" + subval)
    return (SUBFIELD_SEP.join(parts) + SUBFIELD_SEP if parts else ""), pos2 + length
...
FIELD_DESCRIPTIONS = {
    ...
    34: "Acceptance Environment Data (TLV Format)",
    35: "Track 2 Data",
    36: "Track 3 Data",
    ...
    104: "Transaction Description and Transaction-Specific Data",
    111: "Additional Transaction Specific Data (TLV Format)",
    123: "Verification Data",
    124: "Additional Data 3",
    125: "Additional Data 4",
    126: "Visa Private-Use Fields (VSDC / Issuer Script / E-Commerce)",
    127: "File Record(s): Action and Data",
...
# Field 34 dataset-ID -> dataset title (spec pp. 147-160).
FIELD34_DATASET_NAMES = {
    "01": "Authentication Data",
    "02": "Acceptance Environment Additional Data",
    "03": "Additional Service Request Data",
    "04": "Additional Service Result Data",
    "06": "Device Platform Data",
    "07": "Device Common Data",
    "56": "Supplemental Data (tag meanings unconfirmed)",
    "4a": "Strong Customer Authentication and Secure Communication",
}

# Field 34 tag names, keyed by dataset ID, pulled from the spec's
# "Field 34, Dataset ID NN" tables. Dataset 01 tags 86/89/c0 were
# previously confirmed against live samples; the rest (92/93/c1, and all
# of 02/03/04/06/07/4A) are new from the spec text and not yet confirmed
# against live traffic.
FIELD34_TAG_NAMES = {
    "01": {
        "86": "3-D Secure Protocol Version Number",
        "89": "Browser IP Address",
        "92": "3DS App IP Address",
        "93": "Shipping Indicator",
        "c0": "Authentication Program",
        "c1": "Authentication Data Quality Indicator",
    },
    "02": {"80": "Initiating Party Indicator"},
    "03": {"c0": "Account Name Request"},
    "04": {"c0": "Account Name Request Result"},
    "06": {"86": "Device ID", "87": "Device Type (Platform)"},
    "07": {"8d": "SDK App ID (Common)"},
    "4a": {
        "84": "Trusted Merchant Exemption Indicator",
        "87": "Low Value Exemption Indicator",
    },
    "56": {
...
# Field 56 dataset-ID names (spec p. 289). Structure/decode is unconfirmed
# against a live sample - see the FIELD_DEFS[56] comment.
FIELD56_DATASET_NAMES = {
    "01": "Account Data",
    "02": "Contact Information",
    "03": "Customer Identification Data",
    "05": "Account Owner Data",
}

# Per-field dataset-ID name lookup, used by describe_label for the
# dataset-TLV fields (56/104/111/123/127). Falls back to a generic
# "Dataset {id}" label when a field or ID isn't in here.
FIELD_DATASET_ID_NAMES = {
    56: FIELD56_DATASET_NAMES,
}

# Field 44 (Additional Response Data) subfields - names/lengths confirmed
# against live samples for .1-.14; subfield 15 confirmed from spec TOC
# ("Field 44.15 - Primary Account Number, Last Four Digits for Receipt").
FIELD44_DESCRIPTIONS = {
    1: "Response Source / Reason Code",
    2: "Address Verification Result Code",
    3: "Additional Token Response Information",
    4: "Extended STIP Reason Code",
    5: "CVV/iCVV Results Code",
    ...
    12: "Reserved",
    13: "CAVV Results Code",
    14: "Response Reason Code",
    15: "Primary Account Number, Last Four Digits for Receipt",
}

# Field 62 (CPS, bitmap format) subfields - names taken from the Field
# 62.0 bitmap byte tables in the spec (pp. 325-326), which name every bit
# 1-32 even though many are explicitly marked "Not applicable in ATM and
# Auth-Only" for this message family. Byte-lengths for 1-26 are confirmed
# via the per-subfield "Field 62.N - Attributes" sections; 27+ have no
# confirmed length and will show as "Reserved" if ever encountered.
FIELD62_DESCRIPTIONS = {
    1: "Authorization Characteristics Indicator (ACI)",
    2: "Transaction Identifier (TID)",
    3: "Validation Code",
    4: "Market-Specific Data Identifier",
    5: "Duration (not applicable in ATM)",
    6: "Reserved for future use",
    7: "Purchase Identifier (not applicable in ATM)",
    8: "Service Date (not applicable in ATM and Auth-Only)",
    9: "No Show Indicator (not applicable in ATM and Auth-Only)",
    10: "Extra Charges (not applicable in ATM and Auth-Only)",
    11: "Multiple Clearing Sequence Number (not applicable in ATM and Auth-Only)",
    12: "Multiple Clearing Sequence Count (not applicable in ATM and Auth-Only)",
    13: "Restricted Ticket Indicator (not applicable in ATM and Auth-Only)",
    14: "Total Amount Authorized (not applicable in ATM and Auth-Only)",
    15: "Requested Payment Service (not applicable in ATM, Interlink and Auth-Only)",
    16: "Reserved for future use",
    17: "Mastercard Interchange Compliance Info (01xx, authorization only)",
    18: "Excluded Transaction Identifier (not applicable in ATM, Interlink and Auth-Only)",
    19: "Electronic Commerce Goods Indicator (not applicable in ATM and Auth-Only)",
    20: "Merchant Verification Value (MVV) (not applicable in ATM)",
    21: "Online Risk Assessment Risk Score and Reason Codes",
    22: "Online Risk Assessment Condition Codes",
    23: "Product ID (not applicable in Interlink)",
    24: "Program Identifier",
    25: "Spend Qualified Indicator (not applicable in ATM and Interlink)",
    26: "Account Status (not applicable in ATM)",
    27: "ATM Routing Table Unique Identifier (ATM only)",
}

# Field 63 (Visa private-use, bitmap format) subfields. Only bits 1-4
# (byte 1) and bit 3 of byte 3 (= subfield 19) are defined by the Field
# 63.0 bitmap table for this message family (spec p. 362-363) - every
# other bit is spec'd as "n/a, set to zero" for Authorization-Only, so
# it's labeled accordingly below rather than guessed. NOTE: a separate
# byte-length reference table elsewhere in the spec lists subfield 15 as
# 9 bytes, one more than this parser's live-sample-confirmed value of 8 -
# flagged here rather than changed, since shifting a confirmed offset
# risks corrupting every field that follows it. Re-verify against a live
# sample containing F63.15 before touching FIELD63_SUBFIELDS.
FIELD63_DESCRIPTIONS = {
    1: "Network Identification Code",
    2: "Time (Preauthorization Time Limit)",
    3: "Message Reason Code",
    4: "STIP/Switch Reason Code",
    19: "Fee Program Indicator",
}
_FIELD63_UNUSED_DEFAULT = "Not defined for Authorization-Only messages (spec marks this bit n/a)"

# Field 126 (Visa private-use, bitmap format) subfields - names taken
# directly from the Field 126.0 bitmap byte tables (spec pp. 608-610).
# 8/9/10 were previously mislabeled as generic "Issuer Script Data" -
# corrected here to their real spec names (Transaction ID / CAVV Data /
# CVV2 Authorization Request Data). See FIELD126_SUBFIELDS above for
# which of these have confirmed decode lengths.
FIELD126_DESCRIPTIONS = {
    1: "Reserved for future use",
    2: "Reserved for future use",
    3: "Reserved for future use",
    4: "Reserved for future use",
    5: "Visa Merchant Identifier (not applicable in ATM)",
    6: "Cardholder Certificate Serial Number (VSEC, not applicable in ATM)",
    7: "Merchant Certificate Serial Number (VSEC, not applicable in ATM)",
    8: "Transaction ID (XID) (not applicable in ATM)",
    9: "CAVV Data (not applicable in ATM)",
    10: "CVV2 Authorization Request Data",
    11: "Reserved for future use",
    12: "Service Indicators",
    13: "POS Environment (not applicable in ATM)",
    14: "Reserved for future use",
    15: "Mastercard UCAF Collection Indicator (not applicable in ATM)",
    16: "Mastercard UCAF Field (not applicable in ATM)",
    17: "Unused (must not be specified)",
    18: "Agent Unique Account Result",
    19: "Dynamic Currency Conversion Indicator (not applicable in ATM and Interlink)",
    20: "3-D Secure Indicator (not applicable in Interlink and ATM)",
    21: "Reserved for future use",
}
_FIELD126_UNUSED_DEFAULT = "Unused (must not be specified)"


def describe_label(label: str) -> str:
    """
    Resolve a human-readable description for a compact-format label such
    as 'H1', 'F2', 'F34.01.86', 'F55.9f27', 'F60.1', 'F62.2', 'F126.10'.
    """
    ...
    if fnum == 34 and len(parts) == 3:
        ...
        ds_name = FIELD34_DATASET_NAMES.get(dataset, f"Dataset {dataset.upper()} (unconfirmed)")
        tag_name = FIELD34_TAG_NAMES.get(dataset, {}).get(tag, f"Unrecognized/proprietary tag {tag.upper()}")
        return f"{base_desc} - {ds_name} - {tag_name}"

    if fnum in (56, 104, 111, 123, 127) and len(parts) == 3:
        dataset, tag = parts[1], parts[2].lower()
        ds_names = FIELD_DATASET_ID_NAMES.get(fnum, {})
        ds_label = ds_names.get(dataset.lower(), f"Dataset {dataset}")
        return f"{base_desc} - {ds_label}, {EMV_TAG_NAMES.get(tag, 'tag ' + tag.upper())}"

    if fnum == 44 and len(parts) == 2:
        sub = int(parts[1])
        return f"{base_desc} - Subfield {sub}: {FIELD44_DESCRIPTIONS.get(sub, 'Reserved')}"

    if fnum == 60 and len(parts) == 2:
        sub = int(parts[1])
        return f"{base_desc} - Subfield {sub}: {FIELD60_DESCRIPTIONS.get(sub, 'Reserved')}"
    ...
    if fnum == 63 and len(parts) == 2:
        sub = int(parts[1])
        return f"{base_desc} - Subfield {sub}: {FIELD63_DESCRIPTIONS.get(sub, _FIELD63_UNUSED_DEFAULT)}"

    if fnum == 126 and len(parts) == 2:
        sub = int(parts[1])
        return f"{base_desc} - Subfield {sub}: {FIELD126_DESCRIPTIONS.get(sub, _FIELD126_UNUSED_DEFAULT)}"
...
def decode_field_60(data: bytes, pos: int):
    """
    Field 60 (Additional POS Information): 1-byte length = BYTE count
    (private-use convention), followed by up to 6 bytes of packed BCD
    representing 12 fixed digit positions mapped to subfields 60.1-60.10
    (see FIELD60_SUBFIELDS, spec pp. 307-309). Positions are POSITIONAL,
    not bitmap-driven like F44/F62/F63/F126. Per spec, "unused fields
    that precede ones that are used are zero-filled; otherwise, field 60
    is truncated to the last field" - so this stops emitting subfields
    as soon as nibbles run out, rather than assuming a missing trailing
    subfield is zero.
    """
    length, pos2 = read_len_prefix(data, pos)  # byte count
    body = data[pos2:pos2 + length]
    nib = list(nibbles(body))
    parts = []
    npos = 0
    for sub, _name, width in FIELD60_SUBFIELDS:
        if npos >= len(nib):
            break  # remaining subfields not present in this message instance
        take = nib[npos:npos + width]
        npos += width
        digits = "".join(str(n) if n <= 9 else "*" for n in take)
        parts.append(f"F60.{sub})" + digits)
    return (SUBFIELD_SEP.join(parts) + SUBFIELD_SEP if parts else ""), pos2 + length
...
def _split_compact_field(fnum: int, rendered: str, raw_hex: str):
    ...
    """
    rows = []
    if fnum in (34, 44, 55, 56, 60, 62, 63, 104, 111, 123, 126, 127):
...
def parse_message_full(raw_hex: str, debug: bool = False):
    ...
    for fnum in sorted(present_fields):
        ...
        if fnum in (44, 60, 62, 63, 126) or fnum in (34, 55, 56, 104, 111, 123, 127):
...
def write_detail_report(path: str, rows):
    """
    Per-value detail report, format:
        F60.1: Terminal Type

        * Value: `0`
        * Detail: Unspecified. This identifies the basic point-of-service...
    Pulls value meanings from visa_field_details.py.
    """
    with open(path, "w", encoding="utf-8") as f:
